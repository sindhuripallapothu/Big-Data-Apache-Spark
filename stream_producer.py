...
def connect_kafka_producer():
    ...
    try:
        ...
        return _producer
def lines_file(fname):
    num = 0
    with open(fname, 'r') as f:
        for line in f:
            num += 1
    print("No. of lines in file: ", num)
...
if __name__ == "__main__":
    ...
    url = 'http://content.guardianapis.com/search?from-date=' + fromDate + '&todate=' + toDate + '&order-by=newest&show-fields=all&page-size=200&%20num_per_section=10000&api-key=' + key
    all_news = getData(url)
    #with open('/home/sindhuri/Downloads/outFile_training.txt', 'a') as f:
    #    for item in all_news:
    #     item_str = str(item.encode("utf-8"))    +    '\n'
    #     f.write(item_str[2:])
    #f.write(item)
    #print(len(all_news))
    #lines_file('/home/sindhuri/Downloads/outFile_training.txt')
